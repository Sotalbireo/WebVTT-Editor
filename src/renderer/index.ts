...
class VideoController {
	...
	public formatTime(t: number, f = false) {
	...
	public getCurrentTime = (indent = 0) => this.video.currentTime + indent;
	...
	public reloadResource = () => this.video.load();

	public seek = (s: number) => {
		this.video.currentTime += s;
	}

	public seek_f = (f: number) => {
		this.video.pause();
...
class Consoles {
	...
	public resize() {
		const d = document.getElementById('TextareaFrame')!;
...
class Emmy {
	...
	private writeFile(flag= false) {
		const data = (document.getElementById('Textarea')! as HTMLTextAreaElement).value;
